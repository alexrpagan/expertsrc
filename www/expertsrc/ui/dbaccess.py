...
def update_prices():
    ...
    for domain in domains:
        ...
        for level in prices:
            ...
            price = prices[level]
            level = ui.models.Level.objects.get(domain=domain,
...
def max_conf(domain_id, max_price, number_of_questions):
    """
    For all questions in a batch, select (in order) the allocation having the
    maximal confidence which also has a price lower than the one specified.
    """
    ...
    return optimized_alloc_selector(domain_id, get_frame, avg_price,
                                    number_of_questions, frame_size)


def min_price(domain_id, min_conf, number_of_questions):
    """
    For all questions in a batch, select (in order) the allocation having the
    minimal price which meets the given confidence requirements.
    """
    get_frame = """ SELECT confidence_score, user_id_set, price
                    FROM allocs_joined
                    WHERE domain_id = %s
                    ...
                    LIMIT %s """
    frame_size = 2 * number_of_questions
    return optimized_alloc_selector(domain_id, get_frame, min_conf,
                                    number_of_questions, frame_size)
...
@transaction.commit_on_success
def optimized_alloc_selector(domain_id, get_frame_cmd, constraint,
                             number_of_questions, frame_size):
    ...
    alloc = []
    tapped_out = set()
    curr_question = frame_position = workers_left = 0
    ...
    while True:
        ...
        if frame_position == len(frame):
            parms = (domain_id, constraint,
                     list(tapped_out), offset, frame_size,)
...
@transaction.commit_on_success
def get_allocations_by_domain(domain_id, sample_size=10, min_size=1,
                              max_size=7, min_confidence=50, max_price=1000):
    cursor = connection.cursor()
    cmd = """ SELECT c.confidence_score, array_accum(a.user_id) user_id_set,
                     sum(l.price) price
    ...
    vals = (domain_id, min_confidence, max_price,
            min_size, max_size, sample_size,)
    cursor.execute(cmd, vals)
    allocations = list()
    for rec in cursor.fetchall():
        a = Allocation()
...
def denormalize_allocs():
    cmd = """ DROP TABLE IF EXISTS allocs_joined;
              SELECT c.confidence_score, array_accum(a.user_id) user_id_set,
                     sum(l.price) price, c.domain_id
              INTO allocs_joined
             ...
             CREATE INDEX allocs_joined_confidence_score_idx
                       ON allocs_joined ( confidence_score DESC );
             CREATE INDEX allocs_joined_price_idx
                       ON allocs_joined ( price ASC );"""
    cursor = connection.cursor()
    cursor.execute(cmd)
    connection.commit()
